...
def train_step(state: TrainState, 
               t: Array,
               ext_state:optax.OptState,
               optimiser_ext:optax.GradientTransformation
               ) -> Tuple[TrainState, Array, Array,optax.OptState,optax.GradientTransformation]:
    ...
    def physics_loss_fn(params: Dict,t:Array,T:Array) -> Array:
        ...
        refraction = nmin + (nmax - nmin) * 0.5 * (1.0 - jnp.cos(2.0 * jnp.pi * y_pos))
        ones_arr=jnp.ones_like(t)
        vel=(c*ones_arr)/refraction
        
        F_pred=(x_t_pred/total_time)**2+ (y_t_pred/total_time)**2 - vel**2
    ...
    def constraint_loss_fn(params: Dict) -> Array:
        ...
        x_tmin, y_tmin, x_tmax, y_tmax = jnp.array([xmin]), jnp.array([ymin]), jnp.array([xmax]), jnp.array([ymax])
    ...
    def goal_loss_fn(T:Array) -> Array:
        #t_target=jnp.zeros_like(params['T'])
        return T
    
    def loss_fn(params: Dict,t:Array,T:Array) -> Tuple[Array, Array]:
        L_phy = physics_loss_fn(params,t,T)
        L_cons = constraint_loss_fn(params)
        L_goal = goal_loss_fn(T)
    ...
    T=ext_state.params
    (loss, component_loss), grad = jax.value_and_grad(loss_fn, argnums=0,has_aux=True)(state.params,t,T)
    state = state.apply_gradients(grads=grad)
    grad_T=jax.grad(loss_fn,argnums=2)(state.params,t,T)
    updates, ext_state = optimiser_ext.update(grad_T, ext_state, ext_state.params)
    ext_state.params = optax.apply_updates(ext_state.params, updates)
    return state, loss,component_loss,ext_state,optimiser_ext
...
def train_epoch(
    ...
    batch_loss = []
    batch = ts[perms]
    state, loss,component_loss,ext_state,optimiser_ext = train_step(state, batch,ext_state,optimiser_ext)
    batch_loss.append(loss)
    return state, batch_loss,component_loss,ext_state,optimiser_ext
...
def run_pinn_training(
    ...
    optimiser_ext=optax.adam(learning_rate=LR)
    opt_state_ext = optimiser_ext.init(jnp.array([T]))
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        state, train_loss,component_loss,opt_state_ext,optimiser_ext = train_epoch(state,ts,batch_size,epoch,epoch_rng,opt_state_ext,optimiser_ext)
